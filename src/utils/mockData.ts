...
import { Student } from '@/pages/admin/students/studentData';
...
export const fetchMockUsers = async (): Promise<ExtendedUser[]> => {
    return new Promise((resolve) => {
        setTimeout(() => resolve([...MOCK_STUDENTS, ...MOCK_TEACHERS]), 300);
    });
};
...
export const createMockStudent = async (student: Partial<Student>): Promise<Student> => {
    return new Promise((resolve) => {
        setTimeout(() => {
            ...
            const newStudent: Student = { ...student, id: newId, studentId: newId } as Student;
            resolve(newStudent);
        }, 500);
    });
};
...
export const updateMockStudent = async (id: string, updates: Partial<Student>): Promise<Student> => {
    return new Promise((resolve) => {
        setTimeout(() => resolve({ ...updates, id } as Student), 500);
    });
};
